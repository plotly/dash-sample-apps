...
app.layout = html.Div(
    children=[
        ...
        html.Div(
            ...
            className="row",
            style={"background-color": "#1A3399", "color": "white", "width": "100%"},
        ),
        # first column
        html.Div(
            children=[
                ...
                dbc.Tooltip(
                    children=[
                        "The input file should contain the dates in the first column and the values in the "
                        'second column, without column headers. The accepted file formats are CSV, XLS, XLSX, and TXT. If "Sample" '
                        "is selected, the results will be generated using a sample time series."
                    ],
                    target="data_tooltip",
                    placement="right",
                    style={
                ...
                dbc.Tooltip(
                    children=[
                        "The data must be positive for any of the following transformations to be applied."
                    ],
                    target="transformation_tooltip",
                    placement="right",
                    style={
                        "line-height": "0.7vw",
                        "font-size": "0.6vw",
                ...
                dbc.Tooltip(
                    children=[
                        "Must be positive. If zero is selected, no differencing is applied."
                    ],
                    target="differencing_tooltip",
                    placement="right",
                    style={
                        "line-height": "0.7vw",
                        "font-size": "0.6vw",
                ...
                dbc.Tooltip(
                    children=[
                        'If "Auto" is selected, the optimal ARIMA order will be automatically determined, and '
                        "the supplied input values of p, d and q will be ignored."
                    ],
                    target="arima_tooltip",
                    placement="right",
                    style={
                        "line-height": "0.7vw",
                        "font-size": "0.6vw",
                ...
                dbc.Tooltip(
                    children=[
                        'If "Auto" is selected, the optimal seasonal order will be automatically determined, '
                        ...
                        "parameters will not be estimated."
                    ],
                    target="seasonal_tooltip",
                    placement="right",
                    style={
                        "line-height": "0.7vw",
                        "font-size": "0.6vw",
                ...
                # checkbox
                dcc.Checklist(
                    id="auto_seasonal_order",
                    options=[{"label": "Auto", "value": "true"}],
                    value=[],
                    style={
                        "margin": "1vw 0vw 0.25vw 3.5vw",
                        "display": "inline-block",
                        "vertical-align": "middle",
                        "color": "#3770B4",
                        "font-size": "90%",
                ...
                dbc.Tooltip(
                    children=[
                        'If "Trend=Constant" the model will include an intercept and no linear time trend.',
                        html.Br(),
                        'If "Trend=Linear" the model will include a linear time trend and no intercept.',
                        html.Br(),
                        'If "Trend=Both" the model will include both an intercept and a linear time trend.',
                        html.Br(),
                        'If "Trend=None" the model will include no intercept and no linear time trend.',
                    ],
                    target="trend_tooltip",
                    placement="right",
                    style={
                        "line-height": "0.7vw",
                        "font-size": "0.6vw",
        ...
        html.Div(
            children=[
                ...
                html.Label(
                    "Estimated Parameters",
                    style={"color": "#2547A1", "margin": "1vw 0vw 0.25vw 0vw"},
                ),
                # table
                html.Div(
                    id="model_estimation",
                ...
                html.Label(
                    "Fitted Values",
                    style={"color": "#2547A1", "margin": "2vw 0vw 0.5vw 0vw"},
                ),
                # plot
                html.Div(
        ...
        html.Div(
            children=[
                ...
                html.Label(
                    ...
                    style={"color": "#2547A1", "margin": "1vw 0vw 0.5vw 0vw"},
                ),
                # plot
                html.Div(
                    id="residuals_acf",
                ...
                html.Label(
                    ...
                    style={"color": "#2547A1", "margin": "0.25vw 0vw 0.5vw 0vw"},
                ),
                # plot
                html.Div(
                    id="residuals_probplot",
                    children=[
                        # placeholder
                        html.Div(
                            children=["Updating..."],
...
@app.callback(
    [
        Output("model_estimation", "children"),
        Output("model_predictions", "children"),
        Output("residuals_acf", "children"),
        Output("residuals_probplot", "children"),
    ],
    [
        ...
        Input("auto_arima_order", "value"),
        Input("p", "value"),
        Input("d", "value"),
        Input("q", "value"),
        Input("auto_seasonal_order", "value"),
        Input("P", "value"),
        Input("D", "value"),
        Input("Q", "value"),
        Input("s", "value"),
...
def update_results(
    ...
    if sample_data == ["true"] or uploaded_file is not None:
        ...
        if sample_data == ["true"]:
            ...
            df = pd.read_csv(DATA_PATH.joinpath("sample_data.csv"), header=None)
        ...
        try:
            ...
            y = pd.Series(
                ...
                index=pd.DatetimeIndex(
                    pd.to_datetime(df.iloc[:, 0], infer_datetime_format=True)
                ),
            ).dropna()

        except:

            model_estimation = html.Div(
                children=[
                    html.Span(
                        "Error: ",
                        style={
                    ...
                    "The input file should contain the dates in the first column and the values ",
                    html.Br(),
                    "in the second column, without column headers. The accepted file formats ",
                    html.Br(),
                    "are CSV," " XLS, XLSX, and TXT.",
                ],
                style={
                    "color": "#9d9d9d",
                    "font-size": "90%",
            ...
            if y.min() > 0:
                ...
                model_estimation = html.Div(
                    children=[
                        ...
                        "The data must be positive for the selected transformation to be applied.",
                    ],
                    style={
                        "color": "#9d9d9d",
                        "font-size": "90%",
        ...
        try:
            ...
            if (
                auto_arima_order == ["true"]
                and auto_seasonal_order == ["true"]
                and s > 1
            ):
                ...
                if trend == "n":
                    ...
                    res = SARIMAX(
                        ...
                        trend="n",
                        order=opt.order,
                        seasonal_order=opt.seasonal_order,
                    ).fit(disp=0)
            ...
            # series is non-seasonal (s = 1), and therefore the seasonality will be ignored
                ...
                if trend == "n":
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="n", order=opt.order, seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="c", order=opt.order, seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
                    ...
                    # find the optimal order
                    opt = auto_arima(
                        y=y, seasonal=False, with_intercept=False, trend="t"
                    )

                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="t", order=opt.order, seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
                    ...
                    # find the optimal order
                    opt = auto_arima(
                        y=y, seasonal=False, with_intercept=True, trend="ct"
                    )
            ...
            # (this option is not recommended)
                ...
                if trend == "n":
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="n", order=opt.order, seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="c", order=opt.order, seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="t", order=opt.order, seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
            ...
            # is non-seasonal (s = 1), and therefore the seasonality will be ignored
                ...
                if trend == "n":
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="n", order=(p, d, q), seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="c", order=(p, d, q), seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
                    ...
                    # estimate the model
                    res = SARIMAX(
                        endog=y, trend="t", order=(p, d, q), seasonal_order=(0, 0, 0, 0)
                    ).fit(disp=0)
            ...
            # the user has indicated that the time series is seasonal (s > 1)
                ...
                if trend == "n":
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="n", order=(p, d, q), seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
                    ...
                    # estimate the parameters
                    res = SARIMAX(
                        endog=y, trend="c", order=(p, d, q), seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
                    ...
                    # estimate the model
                    res = SARIMAX(
                        endog=y, trend="t", order=(p, d, q), seasonal_order=(P, D, Q, s)
                    ).fit(disp=0)
            ...
            results = pd.DataFrame(
                data={
                    ...
                    "Residual": residuals,
                    "95% CI (Lower Bound)": predictions.conf_int(alpha=0.025).iloc[
                        :, 0
                    ],
                    "95% CI (Upper Bound)": predictions.conf_int(alpha=0.025).iloc[
                        :, 1
                    ],
                },
                index=y.index,
            ...
            table_1 = pd.read_html(
                res.summary().tables[0].as_html(), header=None, index_col=None
            )[0]
            table_2 = pd.read_html(
                res.summary().tables[1].as_html(), header=None, index_col=None
            )[0]
            table_3 = pd.read_html(
                res.summary().tables[2].as_html(), header=None, index_col=None
            )[0]
            ...
            table_1.iloc[:, 3] = (
                table_1.iloc[:, 3].astype(float).apply(lambda x: format(x, ".4f"))
            )

            # organize the second table
            table_2 = table_2.iloc[:, [0, 1, 2, 4]].rename(columns={4: 3})
            table_2.iloc[1:, 1] = (
                table_2.iloc[1:, 1].astype(float).apply(lambda x: format(x, ".4f"))
            )
            table_2.iloc[1:, 2] = (
                table_2.iloc[1:, 2].astype(float).apply(lambda x: format(x, ".4f"))
            )
            table_2.iloc[1:, 3] = (
                table_2.iloc[1:, 3].astype(float).apply(lambda x: format(x, ".4f"))
            )

            # organize the third table
            table_3.iloc[:, 1] = (
                table_3.iloc[:, 1].astype(float).apply(lambda x: format(x, ".4f"))
            )
            table_3.iloc[:, 3] = (
                table_3.iloc[:, 3].astype(float).apply(lambda x: format(x, ".4f"))
            )
            ...
            model_estimation = html.Div(
                children=[
                    ...
                    dt.DataTable(
                        ...
                        columns=[
                            {"id": str(x), "name": str(x)} for x in list(table.columns)
                        ],
                        ...
                        style_data_conditional=[
                            {"if": {"row_index": 2}, "border-top": "double #9d9d9d"},
                            {
                                "if": {"row_index": table.shape[0] - 4},
                                "border-top": "double #9d9d9d",
                            },
                        ],
                    ),
