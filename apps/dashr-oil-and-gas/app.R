...
#a little bit about the app before you go deeper. The two helper functions, fetch_indivudal and fetch_aggregate
#can be improved in terms of run time. fetch_indivudal is really slow while aggregate is a bit better.
#the callback for the count graph that effects the year slider needs to be fixed. Other than that, enjoy!
#Any suggestions and I am sure there will be will be greatly appreciated. 
#_______________________________________________________________

#setwd('/Users/kevinphan/Desktop/OIL AND GAS/data')
...
county_options <- vector(mode="list", length= length(COUNTIES))
for (county in 1:length(COUNTIES)) {
  label1 <- unname(COUNTIES[county])
  value1 <-  labels(COUNTIES[county])
  tmp <- list(label=label1, value=value1)
  county_options[[county]] <- tmp
}

well_status_options <- vector(mode="list", length= length(WELL_STATUSES))
for (well_status in 1:length(WELL_STATUSES)) {
  label1 <- unname(WELL_STATUSES[well_status])
  value1 <-  labels(WELL_STATUSES[well_status])
  tmp <- list(label=label1, value=value1)
  well_status_options[[well_status]] <- tmp
}

well_type_options <- vector(mode="list", length= length(WELL_TYPES))
for (well_type in 1:length(WELL_TYPES)) {
  label1 <- unname(WELL_TYPES[well_type])
  value1 <-  labels(WELL_TYPES[well_type])
  tmp <- list(label=label1, value=value1)
  well_type_options[[well_type]] <- tmp
}
# Load data
  ...
  list(
        ...
        htmlDiv(
          list(
            ...
            dccChecklist(
              id='lock_selector',
              options=list(
                list('label'= 'Lock camera', 'value'= 'locked')
                ),
              values=list(),
              className="dcc_control"
            ),
    ...
    htmlDiv(
      list(
        htmlDiv(
          list(
            dccGraph(id='main_graph')
            ),
          className='pretty_container eight columns'
        ),
        htmlDiv(
          list(
            dccGraph(id='individual_graph')
            ),
          className='pretty_container four columns'
        )
        ),
    ...
    htmlDiv(
      list(
        htmlDiv(
          list(
            dccGraph(id='pie_graph')
            ),
          className='pretty_container seven columns'
        ),
        htmlDiv(
          list(
            dccGraph(id='aggregate_graph')
            ),
...
filter_Dataframe <- function(df, 
  ...
  df <- df[df$Well_Status %in% well_statuses & df$Well_Type %in% well_types,]
  df <- df[df$Date_Well_Completed > min_year & df$Date_Well_Completed < max_year, ]
  return(df)
}
...
fetch_individual <- function(api){
  ...
  return(list(index,water[c(unlist(index))], oil[c(unlist(index))], gas[c(unlist(index))]))
}
...
fetch_aggregate <- function(dff,year_slider){
  ...
  gas1 = sum(unlist(gas[c(index)]))
  oil1 = sum(unlist(oil[c(index)]))
  water1 = sum(unlist(water[c(index)]))
             ...
             function(well_statuses, well_types, year_slider,selector, main_graph_layout){
               ...
               return (list(
                 ...
                 'layout'= list(
                   ...
                   titlefont=list(color='#CCCCCC', size='14'),
                   margin=list(
                     'l'=35,
                     'r'=35,
                     'b'=35,
             ...
             function(well_statuses, well_types, year_slider){
               ...
               return (list(
                 ...
                 layout = list(
                   autosize=TRUE,
                   height=500,
                   font=list(color='#CCCCCC', size='7.5'),
                   titlefont=list(color='#CCCCCC', size='14'),
                   margin=list(
                     'l'=35,
                     'r'=35,
