...
theme <- list(              # old      new
  "black1" = "#2D3038",     # #2D3038  #1F2130
  "black2" = "#1D202D",     # #1D202D  #171A29
        ...
        children = dccGraph(
          ...
          figure = list(
            ...
            layout = list(
              ...
              paper_bgcolor = theme$black2,
              plot_bgcolor = theme$black2
            )
          )
        )
      ),
...
update_metric_summary <- function(param) {
  ...
  app$callback(
    ...
    function(interval, stored_data) {
      total_count <- ifelse(interval < max_length, interval, max_length)
      ooc_n <- ifelse(total_count == 0, stored_data[[param]][["ooc"]][[total_count]]*100) # ERROR?
