...
dates <- DT_$y
Dates <- base::as.Date(dates, format='%m/%d/%Y')
labels <-(DT_$x)[1:11]
DT <- DT_[, 3:13]
colnames(DT) <- labels
rownames(DT) <- dates
...
TEXTS <- l.(
  l.(
    ...
    htmlDiv(dccMarkdown(className='six columns', children=" The yield curve shows how much it costs the federal government to borrow
                 ...
                 tomorrow and many years from now. "), style=l.(width='120%'))), 
  ...
  l.(htmlDiv(dccMarkdown(className='six columns', children= "#### Where we stand"), style=l.(width='120%')), #htmlBr(),
    ...
    mediocre growth in the years ahead."), style=l.(width='120%'))), 
  ...
  l.(htmlDiv(dccMarkdown(className='six columns', children=' #### Last time, a puzzle'), style=l.(width='120%')), #htmlBr(),
    ...
    foreign demand.') , style=l.('width'='120%'))), 
  l.(htmlDiv(dccMarkdown(className='six columns', children='#### Long-term rates are low now, too'), style=l.(width='120%')), #htmlBr(),
    ...
    has risen only modestly since.Some economists refer to the economic pessimism as "the new normal."'), style=l.('width'='120%'))), 
  l.(htmlDiv(dccMarkdown(className='six columns', children=' #### Long-term rates are low now, too'),style=l.(width='120%') ), #htmlBr(),
     htmlDiv(dccMarkdown(className='six columns', children= 'Here is the same chart viewed from above.'), style=l.(width='120%')))
)

##INITALIZE THE GRAPH
graph <- DM %>% .[,dim(.)[2]:1] %>% plot_ly(x=rev(labels), y=base::as.Date(dates, format='%m/%d/%Y'),z=., type='surface', 
...
app$callback(
  ...
  function(value)
  {
    ...
    if(v %in% 0:4){
      p <- plotly::add_trace(graph, type='scatter3d', mode='lines', x=x_snd, y=y_snd, z=z_snd, hoverinfo='x+y+z', line=l.(color='#444444'))
     
      
    } else{
 
      p <- plotly::plot_ly(type='contour', x=Dates, y=rev(labels), z=t(DM),  hoverinfo='x+y+z', lighting=l.('ambient'=0.95, 'diffuse'=0.99, 'fresnel'=0.01, 'roughness'=0.01, 'specular'=0.01 ), colorscale=l.(l.(0, 'rgb(230,245,254)'), l.(0.4,'rgb(123,171,203)'), l.(0.8, 'rgb(40,119,174)'), l.(1, 'rgb(37,61,81)')), zmax=9.18, zmin=0, showscale=F,  line=l.(smoothing=1, color='rgba(40,40,40,0.15)'), contours=l.(coloring='heatmap'))
    
    }
    
    p <- p %>% plotly::layout(.,autosize=T, font=l.(size=12, color='#CCCCCC'), margin=l.(t=5, l=5, b=5, r=5), showlegend=F, hovermode='closest',
                              scene=l.(aspectmode='manual', aspectratio=l.(x=2,y=5,z=1.5), camera=l.(up=UPS[[v+1]], center=CENTERS[[v+1]], eye=EYES[[v+1]]),   annotations=l.(l.(
                                showarrow=F,
                                y="2015-03-18",
                                x="1-month",
                                z=0.046,
