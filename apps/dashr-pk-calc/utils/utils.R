...
# Generates flexible # of columns for the data-table
  ...
  for (subject in seq(0, as.numeric(time_points) - 1)) {
    ...
    parentList[[length(parentList)]]$name <- sprintf("Subj%s Conc (uM)", subject + 1)
...
GenerateResultColumn <- function(results) {
  ...
  resultsDf <- as.data.frame(sapply(resultsRaw, function(x) as.numeric(as.character(x))))
...
RowSD <- function(x) {
# RowSD fun for mutating standard devitation
...
# Generates results-table data
  ...
  names(dfResults)[2:ncol(dfResults)] <- names(select(df, -time))[1:(ncol(dfResults)-1)]
  # Set names for subjects

  dfMean <- mutate(dfResults, mean = round(rowMeans(select(dfResults, -parameter)),1)) 
  dfMeanSDev <- mutate(dfMean, stdev = round(RowSD(select(dfResults, -parameter)),2))
