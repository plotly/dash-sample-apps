              ...
              children = htmlDiv(
                ...
                sequences' dropdown has been cleared."
                  ...
                  htmlA(
                    ...
                    href = "data/sequence_viewer_tubulin.fasta",
...
app$callback(
  ...
  display_preloaded_uploaded_warning <-
    function(preloaded, contents) {
...
app$callback(
  ...
  update_sequence <- function(upload_contents, entry, preloaded) {
    ...
    if (!is.null(preloaded[[1]])) {
      protein <- ReadFasta(preloaded)[[entry]]
    }
      ...
      protein = ReadFasta(data, is_datafile = FALSE)[[entry]]
    }
    else {
      return("-")
    }
...
app$callback(
  ...
  edit_coverage <- function(s_nclicks,
                            r_nclicks,
                            clear_coverage,
                            preloaded,
                            current_cov,
    ...
    if (is.null(s_nclicks[[1]]) &&
        is.null(r_nclicks[[1]]) && (grepl("P01308", preloaded))) {
      return(initialCov)
    }

    if (!is.null(r_timestamp[[1]]) &&
        (is.null(s_timestamp[[1]]) ||
         (s_timestamp[[1]] < r_timestamp[[1]]))) {
      return(list())
    }
          ...
          current_cov <- append(current_cov,
                                list(
                                  list(
                                    ...
                                    "color" = color,
                                    "bgcolor" = bgcolor,
                                    "underscore" = ifelse(length(underscore) >
                                                            0, TRUE, FALSE),
                                    "tooltip" = tooltip
                                  )
                                ))
    ...
    current_cov[order( (sapply(current_cov, function(x)
      x$start) ), decreasing = TRUE)]
...
app$callback(
  ...
  update_protein_options <- function(upload_contents, preloaded) {
    ...
    if (!is.null(preloaded[[1]])) {
      ...
      proteins <- ReadFasta(data, is_datafile = FALSE)[[1]]
    }
    else{
    ...
    if (class(proteins) == "list") {
      ...
      for (i in 1:(length(proteins))) {
        dropdown_options <-
          append(dropdown_options, list(list(
            "label" = i, "value" = i
          )))
      }
      return(dropdown_options)
...
app$callback(
  ...
  update_sequence_title <-
    function(seq, entry, preloaded, upload_contents) {
      if (is.null(entry)) {
        return("")
      }
    ...
    if (is.null(v) || length(v) < 2) {
      ...
      if (break_and_return == FALSE) {
        ...
        if (grepl("dna", alphabet)) {
          ...
          subsequence <-
            substr(subsequence, 1, (nchar(subsequence) - nchar(subsequence) %% 3))
        ...
        if (break_and_return == FALSE) {
          ...
          aa_counts <-
            aa_counts[order( (sapply(aa_counts, function(x)
              x[[2]]) ), decreasing = TRUE)]
...
app$callback(
  ...
  update_desc_info <- function(upload_contents, entry, preloaded) {
    ...
    if (!is.null(preloaded[[1]])) {
      ...
      protein <- tryCatch({
        ReadFasta(data, is_datafile = FALSE)[[entry]]
      },
      error = function(e) {
        return("NA")
      })
