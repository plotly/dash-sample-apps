...
paragraph <- htmlDiv(list(htmlDiv(list(
...
), className = 'rowrow2')), className = "product")
...
performance_graph <- htmlDiv(list(
...
hypothetical_graph <- htmlDiv(list(
...
fees_graph <-
  plot_ly(
    x = list("Category Average", "This fund"),
    y = list("2242", "329"),
    marker = list("color" = "#98151B"),
    name = "A",
    type = "bar",
    height = 150,
    width = 340
...
reviews <- htmlDiv(
  list(
    ...
    htmlLi(
      "Calibre Quantitative Equity Group, the fund's advisor, is among the world's largest equity index managers."
    ),
    htmlBr(),
    htmlP(
      "Did you know? The fund launched in 1976 as Calibre First Index Investment
...
fees_text <- htmlDiv(list(htmlDiv(
  list(
    ...
    htmlDiv(list(
      htmlDiv(list(htmlStrong(
        list("Purchase fee")
      )), className = "three columns right-aligned"),
      
      htmlDiv(list(htmlP(list(
        "None"
    ...
    htmlDiv(list(
      ...
      htmlDiv(
        list(
          ...
          htmlP(
            list(
              "We charge a $20 annual account service fee for each Calibre Brokerage Account,
          ...
          htmlP(
            list(
              "We charge participants a $25 annual account service fee for each fund they hold in their Calibre SIMPLE IRA.
          ...
          htmlP(
            list(
              "We charge participants a $15 annual account service fee for each fund they hold in their Calibre 403(b)(7) account.
                   This fee does not apply to members of Flagship Select, Flagship, Voyager Select, and Voyager Services."
            )
          ),
          htmlBr(list()),
          ...
          htmlP(
            list(
              "We charge participants a $20 annual account service fee for each fund they hold in their
...
current_prices_table <-
  generate_table(as.matrix(current_prices_data))
...
historical_prices_table <-
  generate_table(as.matrix(historical_prices_data))
...
avg_returns <- htmlDiv(list(
  htmlH6(
    list('Average annual returns--updated monthly as of 02/28/2018'),
    className = 'subtitle'
  ),
  htmlBr(),
  avg_returns_table
...
after_tax <- htmlDiv(list(
  htmlH6(
    list('After-tax returns--updated quarterly as of 12/31/2017'),
    className = 'subtitle'
  ),
  htmlBr(),
  after_tax_table
...
recent_returns_table <-
  generate_table(as.matrix(recent_returns_data))
...
equity_characteristics_table <-
  generate_table(as.matrix(equity_characteristics_data))
...
expenses <- htmlDiv(list(htmlH6(list('Expenses'), className = 'subtitle'),
                         htmlBr(),
                         expenses_table), className = "six columns")
...
minimums <- htmlDiv(list(htmlH6(list('Minimums'), className = 'subtitle'),
                         htmlBr(),
                         minimums_table), className = "six columns")
...
secondpage_firstrow <- htmlDiv(list(current_prices,
                                    historical_prices), className = "row")



#First Page First Row

firstpage_firstrow <- htmlDiv(list(funds,
                                   performance_graph), className = "row")

#First Page Second Row

firstpage_secondrow <- htmlDiv(list(# htmlBr(),
  hypothetical_graph,
  price_perf), className = "row")

#News and Reviews
news_reviews <- htmlDiv(list(htmlBr(),
                             news,
                             reviews), className = "row")
...
distributions <- htmlDiv(list(htmlBr(),
                              realized,
                              unrealized), className = "row")

# Portfolio&Stock Page First Row

portfolio_firstrow <- htmlDiv(list(stock_graph,
                                   stock_text), className = "row")
...
risk_reward <- htmlDiv(list(
  ...
  htmlBr(),
  htmlImg(
    src = 'assets/risk_reward.png',
    height = "140",
    width = "332"
  )
), className = "six columns")

white_space <- htmlDiv(list(htmlBr()), className = "six columns")

risk_reward_proper <- htmlDiv(list(white_space,
                                   risk_reward), className = "row")

# Expenses Row

expenses_row <- htmlDiv(list(expenses), className = "row")

# Fees and Minimums Row

fees_mins_row <- htmlDiv(list(minimums,
                              fees_bars), className = "row")


after_tax_row <- htmlDiv(list(after_tax), className = "row")


avg_returns_row <- htmlDiv(list(avg_returns), className = "row")

performance_lines_row <- htmlDiv(list(performance_lines_graph), className = "row")


equity_characteristics_row <- htmlDiv(list(equity_characteristics), className = "row")


equity_div_row <- htmlDiv(list(equity_div), className = "row")


dividends_row <- htmlDiv(list(dividends), className = "row")
...
fees_minimums_layout <- htmlDiv(list(overview,
                                     grey_line,
                                     htmlDiv(
                                       list(expenses_row,
                                            fees_mins_row,
                                            fees_text), className = 'subpagefour'
                                     )), className = 'page')
#################################################################################
