...
WORD_EMBEDDINGS <-
  c("wikipedia_3000", "twitter_3000", "crawler_3000")
...
This is the R version of the t-SNE explorer. To view the source code, please visit the [GitHub Repository](https://github.com/plotly/dashr-tsne)"
            ...
            htmlButton('Learn More', id = 'learn-button', n_clicks =
                         0)
        ...
        htmlDiv(
          list(
            htmlDiv(id = 'KL-div', style = list(display = 'none'))
            ,
            htmlDiv(id = 'end-time', style = list(display = 'none'))
            ,
            htmlDiv(id = 'error-message', style = list(display =
                                                         'none'))
...
app$callback(output = list(id = 'footer', property = 'style')
             ,
             params = list(
    ...
    children = list(
      ...
      htmlDiv(
        dccDropdown(
          ...
          placeholder = 'Select a word to display its neighbors',
          value = ''
        ),
        style = list(width = '25vh')
      )
...
#
###############################################################################################################################
app$callback(output = list(id = 'control-panel', property = 'children'),
                   ...
                   list(
                     ...
                     htmlDiv(
                       list(
                         ...
                         ,
                         htmlP(id = 'upload-label-message', style = list('margin-bottom' =
                                                                           '0px'))
                         ,
                         htmlDiv(
...
app$callback(output = list(id = 'palette', property = 'children'),
             params = list(input(id = 'dropdown-mode-choice', property = 'value')),
             function(choice)
             {
               if (choice == 'demo') {
                 return(list(
                   ...
                   htmlDiv(
                     ...
                     style = list(width = '40vh', height = '30vh')
                   )
                 ))
               } else{
                 return("")
...
app$callback(output = list(id = 'demo-graph', property = 'style'),
             params = list(input(id = 'dropdown-mode-choice', property = 'value')),
             function(value) {
               if (value == 'demo') {
                 return(NULL)
...
app$callback(output = list(id = 'custom-graph', property = 'style'),
             params = list(input(id = 'dropdown-mode-choice', property = 'value')),
             function(value) {
               if (value == 'custom') {
                 return(NULL)
...
datLst <<-
  list(
    mnist_3000 = fread("data/mnist_3000_input.csv.gz", header = T),
    fashion_3000 = fread("data/fashion_3000_input.csv.gz", header =
                           T),
    cifar_gray_3000 = fread("data/cifar_gray_3000_input.csv.gz", header =
                              T),
    wikipedia_3000 = fread("data/wikipedia_3000.csv.gz", header =
                             T),
    crawler_3000 = fread("data/crawler_3000.csv.gz", header =
                           T),
    twitter_3000 = fread(
      'data/twitter_3000.csv.gz',
      encoding = 'Latin-1',
      header = T
    )
  )
...
datLabels <<- list(
  mnist_3000 = unique(fread(
    "data/mnist_3000_labels.csv.gz", header = T
  )),
  fashion_3000 = unique(fread(
    "data/fashion_3000_labels.csv.gz", header = T
  )),
  cifar_gray_3000 = unique(fread(
    "data/cifar_gray_3000_labels.csv.gz", header = T
  )),
...
app$callback(output = list(id = 'div-wordemb-controls', property = 'style')
             ,
...
#
app$callback(output = list(id = 'dropdown-word-selected', property = 'disabled')
             ,
...
#
app$callback(output = list(id = 'dropdown-word-selected', property = 'options')
             ,
             params = list(input(id = 'dropdown-dataset', property = 'value'))
             ,
             function(dataset) {
...
#
generate_figure_word_vec <-
  function(embedding_df,
...
app$callback(output = list(id = 'tsne-3d-plot', property = 'figure')
             ,
             params = list(
             ...
             function(dataset,
               ...
               if (dataset == '' || is.null(dataset)) {
                 ...
                 path <-
                   as.character(
                     glue(
                       'demo_embeddings/{dataset}/iterations_{iterations}/perplexity_{perplexity}/pca_{pca_dim}/learning_rate_{learning_rate}/data.csv'
                     )
                   )
                 ...
                 if (is.element(dataset, IMAGE_DATASETS)) {
                   ...
                   figure <-
                     generate_figure_word_vec(embedding_DT,
                                              wordemb_display_mode,
                                              selected_word,
                                              datTbl)
...
app$callback(output =  list(id = 'div-plot-click-wordemb', property = 'children')
             ,
             params = list(
             ...
             function(clickData, dataset) {
               if (dataset %in% WORD_EMBEDDINGS) {
                 ...
                 p <-
                   plot_ly(type = 'bar',
                           y = neighbors_label$label,
                           orientation = 'h') %>% layout(xaxis = list(title = 'Euclidean Distance'))
                 if (selected_word == '' ||
                     is.na(selected_word)) {
                   toplabel <- "Default 5-NN graph"
                 }
                 else{
                   toplabel <- paste("The 5 nearest neighbors of", selected_word)
                 }
                 return(list(
                   htmlH5(toplabel, style = list(float = 'center')),
                   dccGraph(
...
#
####
app$callback(output = list(id = 'div-plot-click-image', property = 'children')
             ,
             params = list(
               input(id = 'tsne-3d-plot', property = 'clickData')
               ,
             ...
             function(clickData,
               ...
               if (dataset %in% IMAGE_DATASETS) {
                 ...
                 imageIndex <-
                   which(
                     embedding_DT$x == clickPoint[1] &
                       embedding_DT$y == clickPoint[2] & embedding_DT$z == clickPoint[3]
                   )
                 ...
                 if (dataset == 'cifar_gray_3000') {
                   imageMtx <-
                     matrix(imageVec, nrow = 32)
                 } else {
                   imageMtx <- matrix(imageVec, nrow = 28)
                 }
...
parse_contents <- function(contents, filename) {
  ...
  decoded <-
    jsonlite::base64_dec(contents_parsed[[1]][2]) %>% rawToChar(.)
  ...
  return(list(
    dt = DT,
    message = glue('Data {filename} has been successfully uploaded')
  ))
}
...
#
app$callback(output = list(id = 'data-df-and-message', property = 'children')
             ,
             params = list(
...
app$callback(output = list(id = 'upload-data-message', property = 'children')
             ,
             params = list(input(id = 'data-df-and-message', property = 'children'))
             ,
             function(JSON) {
               if (is.na(JSON)) {
...
app$callback(output = list(id = 'label-df-and-message', property = 'children')
             ,
             params = list(
...
app$callback(output = list(id = 'upload-label-message', property = 'children')
             ,
             params = list(input(id = 'label-df-and-message', property = 'children'))
             ,
             function(JSON) {
               if (is.na(JSON)) {
                 return("NO DATA LOADED")
...
app$callback(output = list(id = 'custom-container', property = 'children')
             ,
             params = list(
...
app$callback(output = list(id = 'KL-div', property = 'children')
             ,
             params =  list(input(id = 'custom-container', property = 'children'))
             ,
             function(JSON) {
               L <- fromJSON(JSON)
               return(fromJSON(L[1]))
...
app$callback(output = list(id = 'end-time', property = 'children')
             ,
             params =  list(input(id = 'custom-container', property = 'children'))
             ,
             function(JSON) {
...
app$callback(output = list(id = 'error-message', property = 'children')
             ,
             params =  list(input(id = 'custom-container', property = 'children'))
             ,
             function(JSON) {
...
app$callback(output = list(id = 'tsne-3d-plot-custom', property = 'figure')
             ,
             params =  list(input(id = 'custom-container', property = 'children'))
             ,
             function(JSON) {
