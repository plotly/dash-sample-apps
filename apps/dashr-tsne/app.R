...
Card <- function(kids, ...){
  ...
  return(
    htmlSection(children = kids, style = kwargs)
  )
}
  ...
  htmlDiv( 
    ...
    children = list(
      ...
      htmlDiv(
        className = 'row', 
        children = list(
          htmlH2('t-SNE Explorer', 
                 className = 'title', 
                 id = 'app-title'),
          htmlImg(id = 'plotly-image', 
                  src = 'assets/logo.png', 
                  style = list(height = '90px', 
                             float = 'right', 
                             'margin-top' = '10px', 
                             'margin-right' = '30px'))
...
This is the R version of the t-SNE explorer. To view the source code, please visit the [GitHub Repository](https://github.com/plotly/dashr-tsne)"),
                ...
                dccDropdown(
                  ...
                  options = list(
                              list(label = 'Pre-generated data', 
                                   value = 'demo'),
                              list(label = 'Custom data', 
                                   value = 'custom')
                  ),
                  value = 'demo',
                  clearable = FALSE,
                ...
                htmlDiv(className = 'row, background', 
                        list(   
                          ...
                          htmlDiv(className = 'three columns', 
                                  id = 'control-panel', 
                                  children = list(), 
                ...
                htmlDiv(
                  ...
                  children = dccGraph(id = 'tsne-3d-plot', 
                                      figure = defaultPlot),
                ),
...
demoPanel <- list(   
  ...
  namedSlider("Perplexity",'perplexity', 3, 100,  30, 
              list('3' = list(label = '3'), 
                   '10' = list(label = '10'), 
                   '30' = list(label = '30'), 
                   '50' = list(label = '50'), 
  ...
  namedSlider("Initial PCA dimensions",'pca-dimensions', 25, 100,  50, 
              list('25' = list(label = '25'), 
                   '50' = list(label = '50'), 
                   '100' = list(label = '100')
                  )
              ),
  namedSlider("Learning Rate",'learning-rate', 10, 200,  100, 
              list('10' = list(label = '10'), 
                   '50' = list(label = '50'), 
                   '100' = list(label = '100'), 
                   '200' = list(label = '200')
                  )
...
app$callback(
  ...
  function(choice){
    if(choice == 'demo'){
      ...
      return(
        list(
          htmlDiv(id = 'data-df-and-message', 
                  children = '', 
                  style = list(display = 'none')
          ),
          htmlDiv(id = 'label-df-and-message', 
                  children = '', 
                  style = list(display = 'none')
          ),
          ...
          htmlDiv(list(
            htmlP(id = 'upload-data-message', 
                  style = list('margin-bottom' = '0px')
            ),
            htmlP(id = 'upload-label-message', 
                  style = list('margin-bottom' = '0px')
            ),
            htmlDiv(
...
datLst <<- list(
  mnist_3000 = fread("data/mnist_3000_input.csv", header = T), 
...
generate_figure_word_vec <- function(embedding_df, wordemb_display_mode, selected_word, datTbl){
  ...
  # Nearest Neighbors display only the 100 nearest neighbors of the selected_word, in text rather than circles
    ...
    if(selected_word == ''){
      return(
        plot_ly(
          type = 'scatter3d', x = 0, y = 0, z = 0, mode = 'text', text = 'NA'
        )
      )
    }
    plot_mode <- 'text'
...
app$callback(
  ...
  params = list(input(id = 'tsne-3d-plot', property = 'clickData'),
  ...
  function(clickData, dataset){
    if(dataset %in% WORD_EMBEDDINGS){
      ...
      if(selected_word == '' || is.na(selected_word)){
      ...
      return(
        list(
          htmlH5(toplabel, 
                 style = list(float = 'center')), 
          dccGraph(
            id = 'graph-bar-nearest-neighbors-word',
            figure = p,
            style = list(height = '25vh'),
            config = list('displayModeBar' = F)) %>% htmlDiv(., style = list(height = '25vh', 
                                                                           display = 'block', 
                                                                           margin = 'auto'))
        )
      )
  ...
  function(clickData, dataset, iterations, perplexity, pca_dim, learning_rate){
    ...
    if(dataset %in% IMAGE_DATASETS){
      ...
      if(dataset == 'cifar_gray_3000'){ 
        imageMtx <- matrix(imageVec, nrow = 32)
      } else {
...
app$callback(
  ...
  params = list(input(id = 'upload-label', property = 'contents'),
...
app$callback(
  ...
  params = list(input(id = 'tsne-train-button', property = 'n_clicks'),
