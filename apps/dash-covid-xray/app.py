...
from skimage import draw, segmentation, filters, exposure, color, measure
...
fig_mesh = go.Figure()
fig_mesh.add_trace(
    go.Mesh3d(x=z, y=y, z=x, opacity=0.2, i=k, j=j, k=i)
)
    ...
    [
        ...
        html.Div(
            [
                ...
                html.H6('Slide to find the occlusion and draw a path around its contour'),
            ],
        ...
        html.Div(
            [
                ...
                html.H6('Draw a rectangle to determine the min and max height of the occlusion'),
            ],
            className="app-background",
        ),
         html.Div(
...
def update_segmentation_slices(selected, annotations, fig_mesh3d):
    ...
    if (
        ...
        if len(fig.data) > 1:
            ...
            fig.add_trace(trace)
        t_start = time()
        slices_dict = {str(i): array_to_data_url(img_mask_color[i]) for i in range(top, bottom)}
        t_end = time()
