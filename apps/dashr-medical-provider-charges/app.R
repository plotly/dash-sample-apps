...
dataList <- lapply(state_list, function(st) {
...
init_region <- as.list(
  unique(dataList[[state_list[2]]][, "Hospital Referral Region (HRR) Description"]))[[1]]
...
generateGeoMap <- function(geo_data, selected_metric,
  ...
  string_filter <- "Hospital Referral Region (HRR) Description"
  filtered_data <- geo_data[geo_data[[string_filter]] %in% region_select]
  ...
  plot_mapbox(
    ...
    layout(
      ...
      mapbox = list(
        ...
        pitch = 5,
        zoom = 5,
        # If graph not rendered, try `style = "dark"`
...
generateProcedurePlot <- function(raw_data, cost_select,
  ...
  string_procedure <- "Hospital Referral Region (HRR) Description"
  procedure_data <- raw_data[raw_data[[string_procedure]] %in% region_select]
...
app$callback(
  ...
  function(procedure_select, geo_select, cost_select, state_select) {
    ...
    if ( (prop_id == "geo-map") &
      ...
      provider_select <- lapply(1:length(geo_select[["points"]]), function(i){
      ...
      for (i in 1:nrow(filtered)) {
        ...
        splitProcedureName <- strsplit(
          as.character(fullProcedureName), " - ", fixed = TRUE)[[1]]
...
app$callback(
  ...
  function(cost_select, region_select, procedure_select, state_select) {
    ...
    if (!is.null(unlist(procedure_select))) {
      for (i in 1:length(procedure_select[["points"]])) {
        point <- procedure_select[["points"]][[i]]
...
app$callback(
  ...
  function(cost_select, region_select, geo_select, state_select) {
    ...
    generateProcedurePlot(state_raw_data,
                          cost_select,
                          region_select,
                          provider_select
    )
