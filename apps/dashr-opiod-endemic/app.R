...
df_full_data$County <-
  paste0(df_full_data$County, ", ", df_full_data$State)
df_full_data <- df_full_data[,-c(1, 3)]
df_full_data$`Age Adjusted Rate` <-
  as.numeric(gsub("[^0-9.]", "", df_full_data$`Age Adjusted Rate`))
df_full_data$Deaths <-
  as.numeric(gsub("[^0-9.]", "", df_full_data$Deaths))
...
mapbox_access_token <-
  "pk.eyJ1IjoieXVua2UwMTIzIiwiYSI6ImNqdm1xc3ZyMjByNm00Nm50MzN1YzNnYncifQ.9RhXTBzJRy1sWeL6tx1ssA"
mapbox_style <-
  "mapbox://styles/yunke0123/cjvo2m87d07mk1cpdbo3lvw3j"
...
app$layout(htmlDiv(
  ...
  children = list(
    ...
    htmlDiv(id = "app-container",
            children = list(
              htmlDiv(
                id = "left-column",
                className = "seven columns",
                children = list(
                  htmlDiv(id = "slider-container",
                          children = list(
                            ...
                            dccSlider(
                              ...
                              max = max(YEARS),
                              value = min(YEARS),
                              marks <- as.list(setNames(as.character(
                                seq(
                                  from = min(YEARS),
                                  to = max(YEARS),
                                  by = 1
                  ...
                  htmlDiv(id = "heatmap-container",
                          children = list(
                            htmlP(
                              glue(
                                "Heatmap of age adjusted mortality rates from poisonings in year {min(YEARS)}"
                              ),
                              id = "heatmap-title"
                            ),
                            dccGraph(id = "county-choropleth",
                                     figure = list(
                                       data = list(
                                         list(
              ...
              htmlDiv(
                ...
                children = list(
                  ...
                  dccDropdown(
                    options = list(
                      list("label" = "Histogram of total number of deaths (single year)",
...
app$callback(output = list("id" = "county-choropleth", "property" = "figure"),
             params = list(
...
app$callback(output = list("id" = "heatmap-title", property = "children"),
             params = list(input("id" = "years-slider", property = "value")),
             
             function(year) {
               # Function prints year selected from range slider
               return(glue(
                 "Heatmap of age adjusted mortality rates from poisonings in year {year}"
               ))
             })

app$callback(output = list(id = "selected-data", property = "figure"),
             params = list(
               input(id = "county-choropleth", property = "selectedData"),
             ...
             function(selectedData, chart_dropdown, year) {
               ...
               if (is.null(selectedData[[1]])) {
                 ...
                 dff <-
                   filter(df_full_data, df_full_data$`County Code` %in% fips)
                 ...
                 if (chart_dropdown != "death_rate_all_time") {
                   ...
                   deaths_or_rate_by_fips <-
                     aggregate(
                       x = dff[[AGGREGATE_BY]],
                       by = list(County = dff$County),
                       FUN = function(x) {
                         sum((x[!is.na(x)]))
                       }
                     )
                   ...
                   deaths_or_rate_by_fips <-
                     filter(deaths_or_rate_by_fips, deaths_or_rate_by_fips[[2]] > 0)
                   deaths_or_rate_by_fips <-
                     deaths_or_rate_by_fips[with(deaths_or_rate_by_fips, order(x)),]
                   deaths_or_rate_by_fips <-
                     filter(deaths_or_rate_by_fips,
                            !is.null(deaths_or_rate_by_fips[2]))
                 ...
                 fig <-
                   ...
                   layout(
                     title = list(glue(
                       "<b>{length(fips)} counties selected"
                     )),
                     paper_bgcolor = "#1f2630",
                     plot_bgcolor = "#1f2630",
                     font = list(color = "#2cfec1"),
