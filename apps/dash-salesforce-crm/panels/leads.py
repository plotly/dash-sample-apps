...
from utils.graphs import leads_choropleth_map, lead_source, converted_leads_count
...
def update_graphs(df, status, period):
    ...
    ## Figures
    fig_lead_source = lead_source(status, df)
    fig_leads_choropleth_map = leads_choropleth_map(status, df)
    fig_converted_leads_count = converted_leads_count(period, df)
