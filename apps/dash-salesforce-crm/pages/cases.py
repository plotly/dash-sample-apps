...
def update_graphs(df, period, origin, priority):
    ...
    ## Figures
    fig_by_account = figs.cases_by_account(df, salesforce_manager)
    fig_pie_chart = figs.cases_pie_chart(df, "Reason", priority, origin)
    fig_pie_chart_h = figs.cases_pie_chart(df, "Type", priority, origin, h_orientation=True)
    fig_by_period = figs.cases_by_period(df, period, priority)
