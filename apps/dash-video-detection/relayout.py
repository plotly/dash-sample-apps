...
fig.add_shape(editable=True,
              x0=-1, x1=0, y0=2, y1=3, name='one',
              xref='x1', yref='y1')

fig.add_shape(editable=True,
              x0=1, x1=2, y0=3, y1=4, name='two',
              xref='x1', yref='y1')
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col([
                    dash_table.DataTable(id='table', data=[], columns=[{'id': c, 'name': c} for c in ['scene', 'time', 'order', 'object', 'xmin', 'xmax', 'ymin', 'ymax']])
                ])
            ]
        ),
        dcc.Store(id='shapes')
...
@app.callback(
    [Output('relayout-data', 'children'),
     Output('layout-data', 'children')],
    [Input('graph', 'relayoutData')],
    [State('graph', 'figure')])
def display_relayout_data(relayoutData, figure):
    return json.dumps(relayoutData, indent=2), json.dumps(figure['layout']['shapes'], indent=2)
...
def update_table(relayout_data, figure, table_data):
    ...
    filtered_table_data = [
        ...
        if not (row['order'] == i and row['time'] == round(curr_time, 6) and row['scene'] == scene)
    ]
