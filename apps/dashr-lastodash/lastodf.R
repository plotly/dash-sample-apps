...
convertLAS <- function(LASpath) {
  ...
  for (section in LASsections[[1]]) {
    ...
    if (startsWith(section, "V")) {
      ...
      tableV <- data.frame(x = contentV)
      tableV <- separate(data=tableV, col=x, into=c("label", "x"), sep="\\.", extra="merge")
      tableV <- separate(data=tableV, col=x, into=c("value", "description"), sep=":", extra="merge")
      LASdata$V <- tableV
    }
    ...
    if (startsWith(section, "W")) {
      ...
      tableW <- data.frame(x = contentW)
      tableW <- separate(data=tableW, col=x, into=c("mnemonic", "x"), sep="\\.", extra="merge")
      tableW <- separate(data=tableW, col=x, into=c("unit", "x"), sep=" ", extra="merge")
      tableW <- separate(data=tableW, col=x, into=c("value", "description"), sep=":", extra="merge")
      LASdata$W <- tableW
    }
    ...
    if (startsWith(section, "C")) {
      ...
      tableC <- data.frame(x = contentC)
      tableC <- separate(data=tableC, col=x, into=c("mnemonic", "x"), sep="\\.", extra="merge")
      tableC <- separate(data=tableC, col=x, into=c("unit", "x"), sep=" ", extra="merge")
      tableC <- separate(data=tableC, col=x, into=c("api code", "curve description"), sep=":", extra="merge")
      LASdata$C <- tableC
    }
    ...
    if (startsWith(section, "A")) {
      ...
      tableA <- read.table(text=contentA, header=TRUE, sep="")
      LASdata$A <- tableA
    }
  }
  return (LASdata)
...
LASdata <- convertLAS("apps/dashr-lastodash/data/alcor2.las")
print(LASdata$A)
